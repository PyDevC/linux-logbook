# Stop Bad command delay in Fedora

In Fedora we have a problem where if we type a wrong command we have to wait for sometime as the package manager tries to search the repository list to find if the current typed command matches anything mentioned in the repo list. This can get annoying, depending on the system and the amount of delay you have.

The fix is easy

we have just have to edit a config file
```bash
sudo nvim /etc/PackageKit/CommandNotFound.conf
```
```conf
...
Search for the line `SoftwareSourceSearch` and change its value to `true`.
